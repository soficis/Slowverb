...
public:
  ...
  template <class T> void load(T &t) { load_binary(&t, sizeof(T)); }

  // WASM32 vs Windows LLP64 compatibility:
  // - Windows: sizeof(long)=4, sizeof(size_t)=8
  // - WASM32:  sizeof(long)=4, sizeof(size_t)=4
  //
  // The only mismatch is size_t: Windows has 8 bytes, WASM32 has 4 bytes.
  // We need to read 8-byte size_t from archive into 4-byte size_t variable.
  //
  // Note: MEMORY64 was disabled because it caused type aliasing issues
  // (unsigned long = size_t in LP64, making separate overloads impossible).
#ifdef __EMSCRIPTEN__
  void load(std::size_t &t) {
    ...
    this->load_binary(&t, sizeof(std::size_t));
  }
#endif
...
public:
  ...
  struct use_array_optimization {
    ...
    struct apply {
      typedef
          typename boost::serialization::is_bitwise_serializable<T>::type type;
    };
#else
