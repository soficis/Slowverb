...
basic_binary_iprimitive<Archive, Elem, Tr>::init() {
  ...
  if (sizeof(int) != size)
    boost::serialization::throw_exception(archive_exception(
        archive_exception::incompatible_native_format, "size of int"));
  this->This()->load(size);
  // Skip sizeof(long) check for WASM64 compatibility (cache has 4-byte long
  // from Windows LLP64, but WASM64 LP64 uses 8-byte long). We handle this
  // mismatch in a load() specialization.
#ifndef __EMSCRIPTEN__
  if (sizeof(long) != size)
    boost::serialization::throw_exception(archive_exception(
        archive_exception::incompatible_native_format, "size of long"));
#else
  (void)size; // Consume the byte, but don't validate
#endif
  this->This()->load(size);
  if (sizeof(float) != size)
    boost::serialization::throw_exception(archive_exception(
        archive_exception::incompatible_native_format, "size of float"));
  this->This()->load(size);
  if (sizeof(double) != size)
    boost::serialization::throw_exception(archive_exception(
        archive_exception::incompatible_native_format, "size of double"));
  ...
  if (1 != i)
    boost::serialization::throw_exception(archive_exception(
        archive_exception::incompatible_native_format, "endian setting"));
}
...
basic_binary_iprimitive<Archive, Elem, Tr>::basic_binary_iprimitive(
    std::basic_streambuf<Elem, Tr> &sb, bool no_codecvt)
    :
#ifndef BOOST_NO_STD_LOCALE
      m_sb(sb), codecvt_null_facet(1), locale_saver(m_sb),
