...
typename LofSpace::DistType HnswDistFunc(const void *point1, const void *point2,
...
public:
  ...
  LofSpace(const DistFunc &dist_func)
      : hnswlib::SpaceInterface<DistT>(), dist_func_(dist_func),
        points_(nullptr) {};
  ...
  DistType GetDistance(const LofPoint *point1, const LofPoint *point2) {
    ...
    return dist_func_(
        *point1 < 0 ? **ThreadLocalTemporaryPoint() : points_[*point1],
        *point2 < 0 ? **ThreadLocalTemporaryPoint() : points_[*point2]);
  }

  virtual size_t get_data_size() { return sizeof(LofPoint); }
  virtual hnswlib::DISTFUNC<DistType> get_dist_func() {
    return HnswDistFunc<LofSpace>;
  }
  virtual void *get_dist_func_param() { return this; }
...
template <class Point, class DistType, class DistFunc> class Lof {
public:
  ...
  void Prepare(Iterator bg, Iterator ed, int k, int m = 64) {
    ...
    std::vector<std::vector<std::pair<DistType, hnswlib::labeltype>>> neighbors(
        points_.size()); // 大きい順
