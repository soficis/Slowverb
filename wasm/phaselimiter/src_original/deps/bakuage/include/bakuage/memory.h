...
template <class T> class AlignedPodVector {
public:
  ...
  AlignedPodVector(size_t len, const T &value)
      : size_(0), allocated_size_(0), data_(nullptr) {
    resize(len);
    for (int i = 0; i < len; i++) {
      data_[i] = value;
    }
  ...
  template <class S>
  AlignedPodVector(std::initializer_list<S> list)
      : AlignedPodVector(list.begin(), list.end()) {}

  AlignedPodVector(const AlignedPodVector &x)
      : size_(0), allocated_size_(0), data_(nullptr) {
    resize(x.size());
    if (size_) {
  ...
  AlignedPodVector(AlignedPodVector &&x)
      : size_(x.size_), allocated_size_(x.allocated_size_), data_(x.data_) {
    x.data_ = nullptr;
    x.size_ = 0;
    x.allocated_size_ = 0;
  }
  ...
  void shrink_to_fit() { DoRealloc(size_); }
...
private:
  ...
  void DoRealloc(size_t new_allocated_size) {
    if (allocated_size_ == new_allocated_size)
      return;
