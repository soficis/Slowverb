...
struct MasteringReference2 {
  struct DistFunc {
    ...
    float operator()(const MasteringReference2 &a,
                     const MasteringReference2 &b) const {
#ifdef BA_SOUND_QUALITY2_KL
      return JensenShannonDistance(a.mean_, a.covariance_, a.inv_covariance_,
                                   b.mean_, b.covariance_, b.inv_covariance_);
#else
      return bakuage::VectorNormDiffL2(a.vec_.data(), b.vec_.data(),
                                       a.vec_.size());
#endif
    }
  ...
  MasteringReference2(const Eigen::VectorXd &_mean,
                      const Eigen::MatrixXd &_covariance)
      : mean_count_(_mean.size()),
        vec_(_mean.size() + (_mean.size() + 1) * _mean.size() / 2) {
    ...
    // regularization
    // (正則化項を1くらいにすると、meanの違いを見分けられなくなる。ゼロに近いモノラルとかを見分けているだけ？)
    // 1e-7とかにしても1e-3と傾向変わらず
    const auto reg = _covariance +
                     1e-3 * Eigen::MatrixXd::Identity(mean_count_, mean_count_);
    ...
    for (int i = 0; i < mean_count_; i++) {
      vec_[mean_idx(i)] =
          10 * std::log10(std::pow(10, vec_[mean_idx(i)] / 10) / (1e-37 + ene));
    }
#endif
  }

  void ResizeMeanOnly() { vec_.resize(mean_count_); }
  ...
  int covariance_idx(int i, int j) const {
    return mean_count_ + (2 * mean_count_ - (i - 1)) * i / 2 + j - i;
  }
...
public:
  ...
  static bool ParseReference(const char *analysis_json,
                             Eigen::VectorXd *output_mean,
                             Eigen::MatrixXd *output_covariance,
                             std::vector<Band> *bands = nullptr) {
    using namespace picojson;
    value v;
    std::string err = parse(v, analysis_json);
    if (!err.empty()) {
      std::cerr << "SoundQualityCalculator AddReference error " << err
                << std::endl;
      return false;
    }
  ...
  void Prepare(ReferenceIt reference_bg, ReferenceIt reference_ed,
               BandIt band_bg, BandIt band_ed) {
    ...
    if (true) {
      ...
      for (int j = 0; j < references[0].mean_count(); j++) {
        // 両方有効にすると成績少し上がる
#if 1
        standard_scaler_scales_[references[0].mean_idx(j)] *=
            std::sqrt(references[0].mean_count());
#endif
#if 1
        standard_scaler_scales_[references[0].covariance_idx(j, j)] *=
            std::sqrt(references[0].mean_count());
#endif
      }
      for (int i = 0; i < references.size(); i++) {
        for (int j = 0; j < references[0].vec_size(); j++) {
          references[i].vec()[j] =
              (references[i].vec()[j] + standard_scaler_shifts_[j]) *
              standard_scaler_scales_[j];
        }
      }
    } else {
      std::fill_n(standard_scaler_scales_.begin(),
                  standard_scaler_scales_.size(), 1);
    }
    ...
    for (int i = 0; i < references.size(); i++) {
      ...
      for (int j = 0; j < references[i].mean_count(); j++) {
        standard_scaler_shifts_[references[i].mean_count() + j] =
            standard_scaler_shifts_[references[i].covariance_idx(j, j)];
        standard_scaler_scales_[references[i].mean_count() + j] =
            standard_scaler_scales_[references[i].covariance_idx(j, j)];
      }
    }
#endif
    ...
    if (true) {
      ...
      const Eigen::BDCSVD<Eigen::MatrixXd> svd_solver(
      ...
      Eigen::VectorXd inv_singular_values =
          (1e-37 * Eigen::VectorXd::Ones(references[0].vec_size()) +
...
#endif
      pca_mat_ =
          inv_singular_values.asDiagonal() * svd_solver.matrixV().transpose();
    } else {
      pca_mat_ = Eigen::MatrixXd::Identity(references[0].vec_size(),
                                           references[0].vec_size());
    }
#endif

    // LOF
    lof_.Prepare(references.begin(), references.end(),
                 std::ceil(0.03 * references.size()));
  ...
  double CalculateDistance(const MasteringReference2 &reference,
                           const MasteringReference2 &target) const {
  ...
  void CalculateSoundQuality(const MasteringReference2 &reference,
                             float *output_sound_quality,
                             float *output_lof) const {
    MasteringReference2 proprocessed = reference;
#ifndef BA_SOUND_QUALITY2_KL
    PreprocessReference(&proprocessed);
#endif
    ...
    if (output_sound_quality) {
      auto it = std::lower_bound(sorted_reference_lofs_.begin(),
                                 sorted_reference_lofs_.end(), lof);
  ...
  void CalculateSoundQuality(const Eigen::VectorXd &mean,
...
private:
  ...
  void PreprocessReference(MasteringReference2 *reference) const {
    ...
    for (int j = 0; j < reference->vec_size(); j++) {
      vec[j] = (reference->vec()[j] + standard_scaler_shifts_[j]) *
               standard_scaler_scales_[j];
    }
    vec = pca_mat_ * vec;
    for (int j = 0; j < reference->vec_size(); j++) {
...
public:
  SoundQuality2Calculator()
      : full_unit_(SoundQuality2CalculatorUnit::kModeFull),
        mean_only_unit_(SoundQuality2CalculatorUnit::kModeMeanOnly) {
    units_[0] = &full_unit_;
...
#ifndef BAKUAGE_DISABLE_TBB
  template <class It> void PrepareFromPaths(It path_bg, It path_ed) {
    ...
    tbb::parallel_for<int>(
        0, references.size(), [&paths, &references, &bands](int i) {
          const auto json_str = bakuage::LoadStrFromFile(paths[i].c_str());
          Eigen::VectorXd mean;
          Eigen::MatrixXd covariance;
          if (!SoundQuality2CalculatorUnit::ParseReference(
                  json_str.c_str(), &mean, &covariance,
                  i == 0 ? &bands : nullptr)) {
            throw std::logic_error(
                std::string("failed to parse analysis json ") + paths[i]);
          }
          MasteringReference2 reference(mean, covariance);
  ...
  void Prepare(ReferenceIt reference_bg, ReferenceIt reference_ed,
    ...
    tbb::parallel_for<int>(
        0, units_.size(), [this, &references, band_bg, band_ed](int i) {
          units_[i]->Prepare(references.begin(), references.end(), band_bg,
                             band_ed);
        });
  ...
  void CalculateSoundQuality(const MasteringReference2 &reference,
    ...
    if (output_sound_quality) {
      auto it = std::lower_bound(sorted_reference_lofs_.begin(),
                                 sorted_reference_lofs_.end(), mean_lof);
      int pos = std::distance(sorted_reference_lofs_.begin(), it);
      *output_sound_quality = 1.0 - 1.0 * pos / sorted_reference_lofs_.size();
    }
    if (output_lof) {
