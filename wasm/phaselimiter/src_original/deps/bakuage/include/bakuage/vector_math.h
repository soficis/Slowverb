...
struct Float16 {
  signed short as_int;
  Float16() : as_int(0) {}
  explicit Float16(float f) : as_int((signed short)f) {}
  operator float() const { return (float)as_int; }
};
typedef Float16 ComplexFloat16[2];
#pragma pack(pop)

template <class T> struct NormType {};
template <> struct NormType<float> {
  typedef float Type;
};
template <> struct NormType<double> {
  typedef double Type;
};
template <> struct NormType<std::complex<float>> {
  typedef float Type;
};
template <> struct NormType<std::complex<double>> {
  typedef double Type;
};
...
template <class Float>
void VectorMadConstantInplace(const Float *x, const Float &c, Float *output,
                              int n);

template <class Float>
void VectorPowConstant(const Float *x, const Float &c, Float *output, int n);

template <class Float> void VectorSqrtInplace(Float *output, int n);
...
template <class Float> Float VectorLInf(const Float *x, int n);

template <class Float>
typename NormType<Float>::Type VectorL2(const Float *x, int n);

template <class Float> Float VectorL2Sqr(const std::complex<Float> *x, int n);

template <class Float> Float VectorSum(const Float *x, int n);

template <class Float> void VectorInvInplace(Float *output, int n);

template <class Float> void VectorSet(const Float &c, Float *output, int n);
...
template <class Float>
void VectorInterpolateHold(const Float *x, int src_n, Float *output,
                           int factor);

template <class Float> void VectorReverseInplace(Float *output, int n);

template <class Float> void VectorReverse(const Float *x, Float *output, int n);

template <class Float> void VectorConjInplace(Float *output, int n);

// memmoveより速いわけではないので、使う必要なし
template <class Float> void VectorMove(const Float *x, Float *output, int n);

template <class Float> void VectorZero(Float *output, int n);

template <class Float> Float VectorDot(const Float *x, const Float *y, int n);

template <class Float>
void VectorReplaceNanInplace(const Float &c, Float *x, int n);

template <class Float> void VectorEnsureNonnegativeInplace(Float *x, int n);
...
template <class Float>
void VectorRealToComplex(const Float *x, const Float *y,
                         std::complex<Float> *output, int n);

template <class Float>
void VectorComplexToReal(const std::complex<Float> *x, Float *output_real,
                         Float *output_imag, int n);

template <class Float>
void VectorConvolve(const Float *x, int nx, const Float *y, int ny,
                    Float *output);
} // namespace bakuage
