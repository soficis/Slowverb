...
public:
  ...
  virtual void
  filter_results(std::vector<std::pair<dist_t, labeltype>> &candidates) = 0;
...
// Specialization for size_t to always write 8 bytes
template <>
inline void writeBinaryPOD<size_t>(std::ostream &out, const size_t &podRef) {
  uint64_t val = podRef;
...
// Specialization for size_t to always read 8 bytes
template <>
inline void readBinaryPOD<size_t>(std::istream &in, size_t &podRef) {
  uint64_t val;
  in.read((char *)&val, 8);
  podRef = (size_t)val;
}

// Specialization for std::atomic<size_t> to always read 8 bytes
template <>
inline void readBinaryPOD<std::atomic<size_t>>(std::istream &in,
                                               std::atomic<size_t> &podRef) {
  uint64_t val;
  in.read((char *)&val, 8);
  podRef = (size_t)val;
}

// Specialization for std::atomic<uint64_t> to always read 8 bytes
template <>
inline void
readBinaryPOD<std::atomic<uint64_t>>(std::istream &in,
                                     std::atomic<uint64_t> &podRef) {
  uint64_t val;
  in.read((char *)&val, 8);
  podRef = val;
}
