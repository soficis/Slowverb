...
void AutoMastering5(std::vector<float> *_wave, const int sample_rate,
  ...
  try {
    ...
    std::vector<bakuage::AlignedPodVector<float>> band_loudnesses;
    {
      ...
      const int shift =
          width / 2; // 本当は75% overlapだけど、高速化のために50% overlap
      ...
      tbb::parallel_for<int>(0, band_loudnesses.size(), [&](int pos_idx) {
        ...
        for (int band_index = 0; band_index < band_count; band_index++) {
          int low_bin_index =
              std::floor(width * bands[band_index].low_freq / sample_freq);
          int high_bin_index = std::min<int>(
              std::floor(width *
                         (bands[band_index].high_freq == 0
                              ? 0.5
                              : bands[band_index].high_freq / sample_freq)),
              spec_len);
    ...
    const auto apply_effect = [](const Effect &effect, const Float *input,
                                 Float *output) {
      for (int i = 0; i < effect.band_effects.size(); i++) {
        ...
        // 元のMsCompressorFilterはmsにするときにこれをかけていた。誤差が問題になるなら考える
        static const float log10_div_20 = std::log(10) / 20;
        ...
        Float side_gain =
            std::exp(log10_div_20 *
                     (band_effect.ms_loudness_mapping(mid_to_side_loudness) -
                      mid_to_side_loudness));

        Float total_loudness_with_side_gain =
            -0.691 +
            10 * std::log10(rms_m + rms_s * bakuage::Sqr(side_gain) + 1e-37);
        Float gain = std::exp(
            log10_div_20 * (mapped_loudness - total_loudness_with_side_gain));
    ...
    // エフェクトパラメータからエフェクト適用後のmean,
    // covと各バンドの平均誤差を計算する
    const auto calc_mean_cov = [apply_effect, band_count, &band_loudnesses](
                                   const Effect *effect,
      ...
      for (int band_index1 = 0; band_index1 < 2 * band_count; band_index1++) {
        for (int band_index2 = band_index1; band_index2 < 2 * band_count;
          ...
          for (int i = 0; i < band_blocks1.size(); i++) {
            ...
            const bool valid =
                (x1 >= threshold1) &
                (x2 >= threshold2); // for easy optimization (not && instead &)
    ...
    const auto calc_eval = [calc_mean_cov, &calculator, &original_mean,
      ...
      /*
      std::cerr << "evaluator start " << eval_count
      ...
      */
      ...
      // 最適解のところで
      // sound_qualityの微分とmseの微分が一致していると仮定すると、 alpha = 0.02
      // / sqrt(mse)になった。
      // mseの許容量(target_mse)からalphaを逆算して決める。
      // target_mseはFLAGS_mastering5_mastering_levelを元に適当に決める
      // betaはエフェクトのパラメータを変えたときにどのくらいmseが変わるかを考えて、alphaに比例させて決める。(paramを1変化させて、特徴量が10dB変化するなら、beta
      // = 10^2 * alpha -> いろいろ試して微調整)
      ...
      const float eval =
          main_eval + alpha * mse + beta * msp + bound_error * 1e4;
      {
        std::lock_guard<std::mutex> lock(eval_mtx);
        eval_count++;
        ...
        int progress_interval = std::max<int>(
            1, FLAGS_mastering5_optimization_max_eval_count / 100);
        if (eval_count % progress_interval == 0 &&
            eval_count < FLAGS_mastering5_optimization_max_eval_count) {
          progress_callback(0.1 +
                            0.5 * eval_count /
                                FLAGS_mastering5_optimization_max_eval_count);
        }
        if (min_eval > eval) {
          min_eval = eval;
    ...
    // エフェクトパラメータ探索
    const auto find_params = [calc_eval, band_count, &zero_params, initial_eval,
                              &lower_bounds, &upper_bounds, &best_params,
                              &min_eval, _wave]() -> EffectParams {
      ...
      // 原点が境界そのものにならないように注意。
      // 最初の分布も重要。
      // boundして最適化すると境界付近の値になりやすい気がする
      // (変換の形的に境界付近の存在確率が高まる？次元の呪い？)
      ...
      if (FLAGS_mastering5_optimization_algorithm == "nm") {
        settings.iter_max =
            FLAGS_mastering5_optimization_max_eval_count / lower_bounds.size();
        success = optim::nm(
            result,
      ...
      for (int i = 0; i < band_count; i++) {
        std::cerr << "optimization solution x " << i << "\t"
                  << result(8 * i + 0) << "\t" << result(8 * i + 1) << "\t"
                  << result(8 * i + 2) << "\t" << result(8 * i + 3)
                  << std::endl;
    ...
    // Log first few params to confirm they are zeros
