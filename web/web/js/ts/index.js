...
var WorkerRunner = class {
  ...
  // 10 minutes for Level 5 mastering
...
async function generateToneReverbIr(reverb) {
  ...
  const decaySeconds = clampNumber(0.1 + reverb.decay * 6, 0.1, 8);
  const roomScale = clampNumber(reverb.roomScale ?? 0.5, 0, 1);
  const scaledDecay = clampNumber(decaySeconds * (0.7 + roomScale * 0.2), 0.1, 8);
